...
import { User, Lock, AlertCircle, WifiOff, Clock, CheckCircle } from 'lucide-react';
...
export default function PlayerLoginPage() {
  ...
  useEffect(() => {
    const savedCredentials = localStorage.getItem('playerLoginCredentials');
    if (savedCredentials) {
      try {
        ...
        localStorage.removeItem('playerLoginCredentials');
      }
    }
  ...
  useEffect(() => {
    ...
    if (state?.message) {
      ...
      // Clear the message after 5 seconds without setTimeout
    ...
    if (err instanceof Error) {
      ...
      if (message.includes('invalid credentials') || message.includes('invalid') || message.includes('credentials')) {
        return {
          ...
          message: 'Email/Nama Alias atau password salah.'
        };
      }
      ...
      if (message.includes('permission') || message.includes('unauthorized') || message.includes('access denied')) {
        return {
          ...
          message: 'Anda tidak memiliki izin untuk mengakses area pemain.'
        };
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await loginMutation({
        ...
        password: password, // Password tidak di-lowercase agar hash tetap valid
      });
      ...
      if (result.success && result.user) {
        ...
        if (result.user.role !== 'player') {
          console.error('❌ Role mismatch: Expected "player", got:', result.user.role);
          setError({
            type: 'INSUFFICIENT_PERMISSIONS',
            message: 'Login ini khusus untuk pemain. Silakan gunakan login admin.'
          });
          setIsLoading(false);
          return;
        }
        ...
        if (rememberMe) {
          localStorage.setItem('playerLoginCredentials', JSON.stringify({
            identifier,
            password
          }));
        } else {
      ...
      <div className="relative w-full max-w-[480px] animate-fade-in mt-[-5vh]">
          ...
          <div className="p-6 sm:p-8 md:p-10">
            ...
            <div className="text-center mb-6 sm:mb-8">
              ...
              <p className="text-gray-400 text-sm sm:text-base font-medium">
                Akses kartu skor turnamen Anda
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
              ...
              <div>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    placeholder="Masukkan email, atau nama alias"
                  />
                </div>
              </div>
              ...
              {/* Success Message */}
              {successMessage && (
                <div className="bg-green-950/50 border-2 border-green-900/50 text-green-400 px-3 sm:px-4 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl flex items-start animate-fade-in">
                  <CheckCircle className="h-5 w-5 mr-2.5 mt-0.5 flex-shrink-0" />
        ...
        <div className="mt-6 sm:mt-8 text-center">
          <div className="flex items-center justify-center space-x-2">
            {/* <img src="/logo-golfscoreid.png" alt="GolfScore ID" className="h-5 w-auto" /> */}
